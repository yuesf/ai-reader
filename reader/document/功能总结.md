...
- **认证与鉴权**
  ...
  - 拦截策略：除公开接口外均需 `Authorization: Bearer <token>`
    - 公开接口：`/v1/auth/**`、`/health`、`GET /v1/reports**`
...
- **字段说明**：
  ...
  - `price`：价格，默认0
  - `report_file_id`：报告文件ID，关联[file_info](/ai-reader/reader/src/main/resources/schema.sql#L15-L15)表的[id](/ai-reader/reader/src/main/resources/schema.sql#L3-L3)字段
